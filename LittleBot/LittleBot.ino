...
int flsensor = A4; // Left Sensor on Analog Pin 4
int mlsensor = A3; // Right Sensor on Analog Pin 3
int msensor = A2; // Middle Sensor on Analog Pin 2
int mrsensor = A1; // Right Sensor on Analog Pin 1
int frsensor = A0; // Middle Sensor on Analog Pin 0

const int whitelevl = 920; // reading level is white if > 920
const int blacklevl = 870; // reading level is black if < 870

int servoPin1 = 11;//declares pin for servo1, LEFT SERVO!!
Servo Servo1;// creates Servo1 object

int servoPin2 = 7;//declares pin for servo2, RIGHT SERVO!!
...
void loop() {
  ...
  if (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) > blacklevl && readQD(mrsensor) > blacklevl && readQD(frsensor) > blacklevl)
  {
    ...
    while(true)
    {
      ...
      if ((readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) > blacklevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) > blacklevl && readQD(msensor) > blacklevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) < whitelevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl))
        {Serial.println("DEBUG: EXITING C1 - TURN RIGHT, SWITCHING TO NEW CASE...");break;} 
    }
  }
  ...
  else if (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) < whitelevl && readQD(mrsensor) > blacklevl && readQD(frsensor) > blacklevl)
  {
    ...
    while(true)
    {
      ...
      if ((readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) > blacklevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) > blacklevl && readQD(msensor) > blacklevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) < whitelevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl))
        {Serial.println("DEBUG: EXITING C2 - TURN RIGHT, SWITCHING TO NEW CASE...");break;}
    }
  }
  ...
  else if (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) < whitelevl && readQD(mrsensor) < whitelevl && readQD(frsensor) > blacklevl)
  {
    ...
    while(true)
    {
      ...
      if ((readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) > blacklevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) > blacklevl && readQD(msensor) > blacklevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) < whitelevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl)) 
        {Serial.println("DEBUG: EXITING C3 - TURN RIGHT, SWITCHING TO NEW CASE...");break;}
    }
  }
  ...
  else if (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) > blacklevl && readQD(mrsensor) > blacklevl && readQD(frsensor) < whitelevl)
  {
    ...
    while(true)
    {
      ...
      if ((readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) > blacklevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) > blacklevl && readQD(msensor) > blacklevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) < whitelevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl))  
        {Serial.println("DEBUG: EXITING C4 - TURN RIGHT, SWITCHING TO NEW CASE...");break;}
    }
  }
  ...
  else if (readQD(flsensor) > blacklevl && readQD(mlsensor) > blacklevl && readQD(msensor) > blacklevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl)
  {
    ...
    while(true)
    {
      ...
      if ((readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) > blacklevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) > blacklevl && readQD(mrsensor) > blacklevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) < whitelevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl))   
      {Serial.println("DEBUG: EXITING C5 - TURN LEFT, SWITCHING TO NEW CASE...");break;}
    }
  }
  ...
  else if (readQD(flsensor) > blacklevl && readQD(mlsensor) > blacklevl && readQD(msensor) < whitelevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl)
  {
    ...
    while(true)
    {
      ...
      if ((readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) > blacklevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) > blacklevl && readQD(mrsensor) > blacklevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) < whitelevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl))   
        {Serial.println("DEBUG: EXITING C6 - TURN LEFT, SWITCHING TO NEW CASE...");break;}
    }
  }
  ...
  else if (readQD(flsensor) > blacklevl && readQD(mlsensor) < whitelevl && readQD(msensor) < whitelevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl)
  {
    ...
    while(true)
    {
      ...
      if ((readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) > blacklevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) > blacklevl && readQD(mrsensor) > blacklevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) < whitelevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl)) 
        {Serial.println("DEBUG: EXITING C7 - TURN LEFT, SWITCHING TO NEW CASE...");break;}
    }
  }
  ...
  else if (readQD(flsensor) < whitelevl && readQD(mlsensor) > blacklevl && readQD(msensor) > blacklevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl)
  {
    ...
    while(true)
    {
      ...
      if ((readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) > blacklevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) > blacklevl && readQD(mrsensor) > blacklevl && readQD(frsensor) < whitelevl) || (readQD(flsensor) < whitelevl && readQD(mlsensor) < whitelevl && readQD(msensor) < whitelevl && readQD(mrsensor) < whitelevl && readQD(frsensor) < whitelevl)) 
        {Serial.println("DEBUG: EXITING C8 - TURN LEFT, SWITCHING TO NEW CASE...");break;}
    }
  }
  else //Case 9 : Default to going straight!
  {
    Serial.println("DEBUG: C9 - DEFAULT, GOING FORWARD.");
    goforward(); // By default, the rover will go forward
  }
}
void goforward(){
...
void turnright(){
  Servo1.write(95);//tells servo1 to go full speed counterclockwise
  Servo2.write(95);//tells servo2 to go full speed clockwise
}

void turnleft(){
  Servo1.write(85);//tells servo1 to go full speed clockwise
  Servo2.write(85);//tells servo2 to go full speed counterclockwise
}
